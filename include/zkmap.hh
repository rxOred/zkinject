...
namespace ZkProcess {
    ...
    class MemoryMap  {
        public:
            ...
            std::optional<addr_t> GetModuleStartAddress(const char 
                    *module_name) const;
            std::optional<addr_t> GetModuleEndAddress(const char *
                    module_name) const;
            std::optional<const page_t> GetModulePage(const char *
                    module_name) const;

            inline const page_t& GetBasePage(void) const
            {
                return  mm_pageinfo[0];
            }
            inline const page_t& GetLastPage(void) const
            {
                return *mm_pageinfo.end();
            }
            inline std::vector<page_t>::const_iterator
            ...
            {
                return std::make_pair(mm_pageinfo.begin(), 
                        mm_pageinfo.end());
            }
            ...
            inline std::vector<page_t> GetMemoryPages(
                    void) const
            {
                return mm_pageinfo;
            }
