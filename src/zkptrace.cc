...
template <typename T>
zkprocess::Ptrace<T>::Ptrace(const char **path,
...
void zkprocess::Ptrace<T>::ptrace_init_from_file(
    const char **path, zktypes::u8_t flags) noexcept {
    p_pid = fork();
    if (p_pid == 0) {
        if (ZK_CHECK_FLAGS(PTRACE_DISABLE_ASLR, flags)) {
            personality(ADDR_NO_RANDOMIZE);  // sudo required
        }
...
template <typename T>
void zkprocess::Ptrace<T>::ptrace_init_from_pid(pid_t pid) {}
