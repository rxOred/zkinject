...
ZkElf::Elf::Elf(const char *pathname, ZkElf::ELFFLAGS flags, ZkLog::Log *log)
    :elf_memmap(nullptr), elf_pathname(pathname), elf_baseaddr(0),
    elf_ehdr(nullptr), elf_phdr(nullptr), elf_shdr(nullptr), elf_size(0),
    elf_dynamic(nullptr), elf_symtab(nullptr), elf_dynsym(nullptr),
    elf_dynstr(nullptr), elf_strtab(nullptr), elf_flags(flags), elf_log(log)
{
    try{
        OpenElf();
        return;
...
bool ZkElf::Elf::OpenElf(void)
{
    ...
    if (elf_pathname == nullptr) {
        if (elf_log != nullptr)
            elf_log->PushLog("pathname is not specified",
...
bool ZkElf::Elf::LoadDynamicData(void)
{
    if (GetElfType() != ET_DYN) {
        if (elf_log != nullptr)
            elf_log->PushLog("file does not have dynamic data",
                         ZkLog::LOG_LEVEL_ERROR);
        return false;
    }
    int dynamic_index = 0;
...
int ZkElf::Elf::GetSegmentIndexbyAttr(u32_t type, u32_t flags, u32_t prev_flags)
    const
{
...
void ZkElf::Elf::SetSectionAddressAlign(int shdr_index, u64_t new_address_align)
{
...
void ZkElf::Elf::SetSectionData(int shdr_index, void *data)
{
    ...
    memcpy(((u8_t *)GetMemoryMap() + offset), data, GetSectionSize(shdr_index));
    autoSaveMemoryMap();
}
...
void ZkElf::Elf::SetSegmentAlignment(int phdr_index, u64_t new_alignment)
{
...
void ZkElf::Elf::SetSegmentData(int phdr_index, void *data)
{
    ...
    memcpy(((u8_t *)GetMemoryMap() + offset), data, GetSegmentFileSize(phdr_index));
    autoSaveMemoryMap();
}
